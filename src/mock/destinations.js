const destinations = [
  {
    'id': '831548a8-6d8f-4058-8903-bd0cc495d7be',
    'description': 'Paris - middle-eastern paradise',
    'name': 'Paris',
    'pictures': []
  },
  {
    'id': '0a264369-b490-4454-9a8b-c1be05e56c6e',
    'description': 'Sochi - a perfect place to stay with a family',
    'name': 'Sochi',
    'pictures': []
  },
  {
    'id': '5e9861dd-5550-4e67-8a4e-17f47ce299dd',
    'description': 'Amsterdam - famous for its crowded street markets with the best street food in Asia',
    'name': 'Amsterdam',
    'pictures': [
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/12.jpg',
        'description': 'Amsterdam with an embankment of a mighty river as a centre of attraction'
      },
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/1.jpg',
        'description': 'Amsterdam in a middle of Europe'
      }
    ]
  },
  {
    'id': '16251cbb-f7d1-471a-abe0-623fbf5987b1',
    'description': 'Madrid - a true asian pearl',
    'name': 'Madrid',
    'pictures': [
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/8.jpg',
        'description': 'Madrid with an embankment of a mighty river as a centre of attraction'
      }
    ]
  },
  {
    'id': '040aaa23-a756-41e2-a4e8-1e6c2a928ad8',
    'description': '',
    'name': 'Oslo',
    'pictures': []
  },
  {
    'id': 'd4f083f7-82cb-4c76-9de4-f0a9545d3003',
    'description': 'Kioto - for those who value comfort and coziness',
    'name': 'Kioto',
    'pictures': [
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/12.jpg',
        'description': 'Kioto a true asian pearl'
      },
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/11.jpg',
        'description': 'Kioto a true asian pearl'
      }
    ]
  },
  {
    'id': 'd548e178-9373-4bba-b8a6-03bbae7fda73',
    'description': 'Den Haag - with crowded streets',
    'name': 'Den Haag',
    'pictures': [
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/11.jpg',
        'description': 'Den Haag with an embankment of a mighty river as a centre of attraction'
      }
    ]
  },
  {
    'id': '5fe8f27e-c1f8-4b6a-90ba-ac59fd55155f',
    'description': 'Frankfurt - is a beautiful city',
    'name': 'Frankfurt',
    'pictures': [
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/14.jpg',
        'description': 'Frankfurt famous for its crowded street markets with the best street food in Asia'
      },
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/14.jpg',
        'description': 'Frankfurt with an embankment of a mighty river as a centre of attraction'
      },
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/20.jpg',
        'description': 'Frankfurt with an embankment of a mighty river as a centre of attraction'
      },
      {
        'src': 'https://22.objects.htmlacademy.pro/static/destinations/1.jpg',
        'description': 'Frankfurt is a beautiful city'
      }
    ]
  },
  {
    'id': '89731585-9320-4a00-89ba-f2667b46bef6',
    'description': '',
    'name': 'Kopenhagen',
    'pictures': []
  },
  {
    'id': '1b941616-3f51-4c24-9a0a-02f829666655',
    'description': '',
    'name': 'Milan',
    'pictures': []
  }
];
